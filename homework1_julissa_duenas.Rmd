...
$L(\lambda, y) = \prod_{i=1}^{n}(\lambda^{y_i}e^{-\lambda})/y_i!$

$L(\lambda, y) = (\lambda^{\sum_{i=1}^{n}yi}e^{-n\lambda})/\prod_{i=1}^{n}y_i!$

$L(\lambda, y) = e^{-n\lambda}\lambda^{\sum_{i=1}^{n}yi}$
...
$\ell(\lambda) = ln(e^{-n\lambda}\lambda^{\sum_{i=1}^{n}yi})$

$\ell(\lambda) = ln(e^{-n\lambda}) + ln(\lambda^{\sum_{i=1}^{n}yi})$

$\ell(\lambda) = \sum_{i=1}^{n}y_i\ln(\lambda) - n\lambda$

Maximum Likelihood Estimate:

$\hat{\lambda} = (\sum_{i=1}^{n}yi)/n$
...
plot+geom_vline(xintercept = (sum(x))/37, color = 'red')
```
...
Our interpretation of $\lambda$ in this model is the expectation of Yi which would be the number of times the word *dark* is used every 1000 words. Our interpretation for vi/1000 is that it is the frequency of the word *dark*.
...
cl <- chapter_lengths
L <- function(lambda){
  (-lambda)*sum(cl)+sum(x)*log(p*sum(cl)/1000)
}
plot2 <- ggplot(data=data.frame(lambda=0),mapping = aes(x=lambda))+stat_function(fun=L)+xlim(0,40)+scale_y_continuous(name = 'Likelihood')
...
It may be unreasonable to assume this because not all chapters contain the same plot points. It may be that one chapter is set during night time and that may up the scale to which 'dark' is being used more frequently than if a chapter was set during the day time.
...
```
Yes there are outliers that don't seem to be reflected in our simulated histogram. In the observed data, there seems to be a more spread out histogram of the dark counts, having an outlier nearing 25 but that is not seen in the simulated histogram
...
y_i is the number of times *dark* is used in the ith chapter
...
Y_i has a Poisson distribution with parameter $\lambda_i$ which as stated before, has a Gamma distribution

We would firstly have to simulate values for $\lambda_i$ in order to do so for Y_i
...
In this case, the variance is greater than the mean, meaning it is overdispersed. This makes sense because we do not have a constant $\lambda$. The rate changes for each chapter.
...
You just showed that a Gamma mixture of Poisson distributions is a negative binomial distribution.  
